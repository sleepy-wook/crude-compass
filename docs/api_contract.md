...
### 4.4-ii `GET /api/market/opec-history?limit=24`

OPEC MOMR 월별 history (default 24개월, max 120). 자료실 (D-1 reports model) 페이지에서 사용.
...
`saudi_delta_vs_prev` = 해당 월 사우디 production - 직전 월. null 가능 (가장 오래된 month).

**Response 200** (signals/contribution):
```json
{
  "window_days": 30,
  "items": [
...
## 7.6 Agent Bricks Supervisor Streaming (D-1, 2026-05-21)

### `POST /api/supervisor/query/stream`

`/api/supervisor/query`의 SSE streaming 버전. 토큰 단위 점진 표시 + sub-agent 호출 시 실시간 이벤트.

**Request body**: same as `/query` (`{ question, mission_id? }`)

**Response**: `text/event-stream` (Server-Sent Events)

각 line `data: {JSON}\n\n` 형식. types:
- `delta` — 답변 토큰 append: `{"type": "delta", "text": "..."}`
- `tool_call` — sub-agent 호출됨: `{"type": "tool_call", "name": "genie-..."}`
- `done` — 완료: `{"type": "done", "answer": "...", "tools_used": [...]}`
- `error` — 실패: `{"type": "error", "message": "..."}`
- `fallback` — Supervisor 미설정: `{"type": "fallback", "reason": "supervisor_not_configured"}` (client는 `/query` 일반 endpoint로 retry 권장)

**Headers**: `Cache-Control: no-cache, no-transform`, `X-Accel-Buffering: no` (reverse proxy buffering 비활성).

매니저가 30~90초 대기 동안 추론 과정 시각 확인용. Investigation 페이지에서 사용.
...
### `POST /api/admin/reports/trigger-now`

매뉴얼 trigger — detector 돌려서 잡힌 거 Haiku-4-5로 보고서 생성 + Lakebase insert.

notebook cron (gdelt 15분 / price 15분 / curation 06:30) + UI smoke 모두 호출.

Query:
- `trigger_type` (optional): `gdelt_signal` | `price_spike` | `pattern_drift`. 생략시 3개 detector 모두 실행 (UI smoke). notebook은 자기 trigger 타입만 emit.
- `force` (default `false`): trigger 0건이어도 dummy event 1개 합성해서 1건 만듦.

**Response 400** (`INVALID_TRIGGER_TYPE`): trigger_type 값 오류.
...
```json
{
  ...
  "results": [
    {
      "fingerprint": "pattern:2026-05-21",
      "ok": true,
      "report_id": "uuid",
      "headline": "공급 위험 지수 급등, 헤지 존 진입",
      "recommendation": "DEFER SPOT"
    },
    {
      "fingerprint": "gdelt:abc123",
      "ok": false,
      "error": "LLM_TIMEOUT: ..."
    }
  ]
...
### `POST /api/admin/daily-report/generate-now`

매뉴얼 daily_report 생성 — 06:30 cron 흐름을 그대로 실행. notebook 없이 UI에서 확인용.

Query:
- `target_date` (default KST 오늘): ISO `YYYY-MM-DD`.
- `overwrite` (default `false`): 기존 row 있으면 삭제 후 새로 생성. `false`면 skip (기존 daily_id 반환).

**Response 200 (success)**: `{ "ok": true, "daily_id": "uuid" }`

**Response 200 (failure)**: `{ "ok": false, "error": "<reason>", "note": "..." }`

**Response 400** (`INVALID_DATE`): target_date ISO 파싱 실패.
...
## 8.8 Reports Endpoints (D-1 추가 — reports model 2026-05-21)

Event-driven AI report inbox. mission 모델 대체. trigger 3종 (gdelt_signal, price_spike, pattern_drift)이 LLM(Haiku-4-5)으로 보고서 생성 → 매니저 inbox.

Status flow: `pending` → `kept` | `dropped` | `ai_dropped`.

### `GET /api/reports/inbox`

pending 상태 보고서 — Decision Room inbox용.
  ...
  "items": [
    {
      ...
      "trigger_meta": {"fingerprint": "...", "headline_hint": "...", "detected_at": "..."},
      "status": "pending",
      "status_changed_at": null,
      "status_changed_by": null,
      "headline": "한 줄 요약",
      "summary": "3줄 요약",
      "reasoning": {"key_signals": [...], "logic": "...", "risk_factors": [...], "recommendation_text": "..."},
...
Lakebase 미연결 시 `{count: 0, items: []}`.

### `GET /api/reports/{report_id}`

단건 + thread (parent_id chain recursive — root + 모든 자손 chronological).
...
404 `NOT_FOUND` — report_id 없음.

### `GET /api/reports/archive`

archive view — `kept` / `dropped` / `ai_dropped` 중 하나.

Query: `status` (default `kept`, enum), `limit` (default 50, max 200)

Response: `{ "status": "kept", "count": N, "items": [Report, ...] }`

### `POST /api/reports/{report_id}/keep`

매니저 보관. status=`kept`, by=`manager`. pulse broadcast `manager:report_kept`.

이미 처리된 보고서 (pending 아님) → `{ok: true, no_change: true, current_status: "..."}` idempotent 응답.

Response 200: `{ "ok": true, "report_id": "...", "new_status": "kept" }`

### `POST /api/reports/{report_id}/drop`

매니저 drop. status=`dropped`, by=`manager`. pulse broadcast `manager:report_dropped`.

Response: `keep`과 동일.

### `POST /api/reports/{report_id}/investigate`

추가 조사 — Agent Bricks Supervisor 호출 (Genie · Knowledge Assistant · 권고 sub-agent cross-check).

Supervisor 응답을 **새 report row로 INSERT** (parent_id=원본) — thread 자식. UI에서는 원본 detail의 "연관 보고서" section에 표시되어 클릭하면 detail 이동.
...
```json
{
  ...
  "status": "completed",
  "new_report_id": "uuid",
  "tools_used": ["genie", "knowledge_assistant", "claude-haiku"],
  "answer": "Supervisor 종합 답변 (400자 미리보기)"
}
```

**Response 200 (Supervisor 미설정)**:
```json
{
  "ok": false,
...
**소요 시간**: 10-30초 (sub-agent fan-out). frontend는 loading state 필요.

---

## 8.9 Daily Reports Endpoints (D-1 추가 — reports model 2026-05-21 Phase 3)

매일 06:30 KST cron이 어제 kept reports + 어제 daily_report + 시장 snapshot → Haiku로 종합 + ratio_suggestion 생성. reference only — 실제 OSP 결재는 매니저.

`daily_reports` Lakebase table 1 row / day (report_date UNIQUE).

DailyReport JSON shape:
```json
{
  "daily_id": "uuid",
  "report_date": "2026-05-21",
  "prev_daily_id": "uuid | null",
  "kept_report_ids": ["uuid", ...],
  "kept_count": 6,
  "kept_summary": "어제 보관된 보고서들 종합 (200자 이내)",
  "prev_daily_summary": "직전 daily_report의 kept_summary",
  "market_context": "현재 시장 상태 + 어제 대비 변화",
  "ratio_suggestion": {
    "direction": "lean_hedge | neutral | lean_opportunity",
    "term_delta_pct": "+5" | "0" | "-5",
    "spot_delta_pct": "-5" | "0" | "+5",
    "qualitative": "방향성 1-2줄 설명",
    "scenarios": [
...
오늘 (KST 기준) daily_report. 없으면 `{"daily_report": null}` graceful (예: 아침 06:30 이전).

Response 200: `{ "daily_report": DailyReport | null }`

### `GET /api/daily-reports/recent`

최근 N일 daily_reports (report_date DESC).

Query: `limit` (default 7, max 30)

Response 200: `{ "count": N, "items": [DailyReport, ...] }`

### `GET /api/daily-reports/{report_date}`

특정 날짜 daily_report. ISO `YYYY-MM-DD` path.

Response 200: `{ "daily_report": DailyReport }`

404 `NOT_FOUND` — 해당 날짜 없음.
