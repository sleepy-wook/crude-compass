...
- **5축 추정 합계: 83 → 92/100** (D-14 다른 채팅 시나리오 통합 흡수 후 update)
- **가장 큰 risk (D-14 update)**: ① Mock backtest 78%/71% 산출 방법 → 5개월 RSS archive backtest 코드 (Sprint 3 ⭐) ② Genie Public Preview → certified queries + canned fallback ③ Supervisor No-code UI 안정성 검증 → Sprint 3 day 1
- **시나리오 보정 필요 사항 8개 → 16개** (D-14 통합 후): 가격 / 페이지 / Pattern Score scale / AIS batch / 데모 단순화 / 일정 / cargo / backtest narrative + GDELT 추가 / OPEC MOMR Document Intelligence / Supervisor + 5 sub-agent / Time Travel 슬라이더 / Confidence Score UI / 랜딩 코스트 / 시간 감쇠 람다 차등 / 평시 가치 6년 그래프 / Track 1 reaffirm
- **MVP scope (D-14)**: 9 Jobs (7 real + 1 optional + 1 mock), Supervisor + 5 sub-agent (1 핵심 Mission Plan + 4 얇은/시연)
- **권장**: 시나리오 v2 (다른 채팅 메인 + 우리 양방향/Slack/Pivot 유지) — Track 1 narrative 강조 + Open Data Democratization
...
### 4.6 5축 종합 (D-14 통합 후 재추정)

| # | 축 | Phase 2 (5/8) | D-14 (5/10 통합) | 추가 leverage |
|---|---|---|---|---|
| 1 | Business Applicability | 17/20 | **18/20** | 평시 가치 narrative + 랜딩 코스트 + 매주 미세 조정 + Confidence Score |
| 2 | Creativity & Innovation | 18/20 | **20/20** | Bidirectional + Open Data Democratization + 시간 감쇠 람다 차등 + Time Travel 람다 비교 + Living Mission Pivot |
| 3 | UX & Insights | 16/20 | **18/20** | Confidence Score UI + Time Travel 슬라이더 + 시그널 기여도 분해 + 6년 평시 그래프 |
| 4 | Technical Capability | 15/20 | **18/20** | Agent Bricks 5 빌드 옵션 모두 + Supervisor + Document Intelligence + GDELT + UC Function 분리 + Lakehouse Sync |
| 5 | Data Storytelling | 17/20 | **18/20** | 호르무즈 + Time Travel 백테스트 + 6년 평시 가치 + Open Data Democratization Track 1 |
| | **합계** | **83/100** | **92/100** | |

> **92/100 도달 사유 (D-14 통합)**: ① Agent Bricks 5 빌드 옵션 모두 활용 (Technical +3) ② 평시 가치 narrative + 랜딩 코스트 (Business +1) ③ Time Travel + Confidence Score (UX +2) ④ 시간 감쇠 람다 차등 + Living Pivot 결합 (Creativity +2) ⑤ 6년 평시 그래프 (Storytelling +1).
>
> **잔여 8점**: Supervisor No-code UI 안정성 검증 / Genie Public Preview 깨짐 risk / 5분 데모 라이브 비율 안정성 / 도메인 검수 (친구분 검수 + 형욱님 부친 인사이트).
