...
[신설 layer — 이 plan]
  ...
  Backend:
    ...
    - app/services/daily_report.py        06:30 aggregation + LLM (Haiku-4-5)
...
- Create: `databricks/notebooks/job_daily_report.py` (06:30 cron, 신규)
  ...
  - Step 3: Foundation Model API call (**Haiku-4-5 고정** — credit 절약) → 종합 + ratio_suggestion JSONB 생성
...
- Modify: `backend/app/services/report_ai_decision.py`
  - 매 trigger 발생 시:
    ...
    2. 각각 새 snapshot과 LLM judge (**Haiku-4-5**) → stale / continuation / unrelated
...
3. ~~**Phase 3 Daily report Foundation Model 선택**~~ — **확정: 모든 report (event + daily) `databricks-claude-haiku-4-5` 사용** (credit 절약, 2026-05-21 결정)
