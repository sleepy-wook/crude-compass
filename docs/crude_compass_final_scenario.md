...
[Apps + Lakebase] (Vite + FastAPI hybrid, Apps 단일 deploy, Lakebase 메모리)
   ...
   └─ [Mission Plan Agent — Foundation Model API ⭐]
        ...
        │
        ├ [Genie Space] — AIS / 가격 / EIA / GDELT 자연어 질의 (live + 4-tier fallback)
...
- `w.serving_endpoints.query()` 직접 호출 (Agent Bricks Custom Agent 미사용)
- 시스템 프롬프트 + Pattern Score + top_signals → JSON output (HEDGE/OPP/Pivot)
- D-3 enhancement: top_signals에 AIS 5척 lifecycle + OilPriceAPI spike 자동 포함

### 9.3 Genie Space — 정형 데이터 자연어
- **테이블** (≤30): Gold daily_risk_score, signal_events_decayed, oil_prices, ais_positions, eia_inventory, missions
- **certified queries**: 두바이 가격 추이, 호르무즈 7일 통과량, 시그널별 기여도 분해, 5척 lifecycle
...
- 권고 따랐을 때 vs 안 따랐을 때: **+0.626% 평균 절감** (v6 backtest 298건 검증)
- v6 backtest 결과: HEDGE 75% hit rate, n=298, 2019-2026

Narrator: "이 backtest는 7년 4개월 stratified 298건. v6 LLM with Recency Weighting + Structured Fields."
...
| 2 | 4 | 자연어 → Genie + Knowledge Assistant + UC Function 종합 응답 (Supervisor 없이 backend orchestration) | 다른 채팅 흡수 |
| 3 | 4 | Slack ↔ Apps 5초 sync 라이브 | 우리 차별화 ⭐ |
| 4 | 4 | Pre-emptive HEDGE Mission Pattern Score 82 | 우리 차별화 ⭐ |
| 5 | 5 | **Backtest 시점 슬라이더** (frontend WhatIf, v6 backtest 298건 75% hit) | 다른 채팅 흡수 |
...
| 5 | Data Storytelling | **15-17/20** | 호르무즈 진행 중 fact + 양방향 timeline + v6 backtest 298건 75% hit + K-Petroleum 5척 실데이터 lifecycle + Open Data Democratization |
| | **합계** | **82-88/100** 정직 | |
...
- ✅ 실제 production-grade: Apps + Lakebase + Genie (live or fallback) + Knowledge Assistant 1개 + Foundation Model API + UC Function + Lakeflow + Document Intelligence + v6 backtest
