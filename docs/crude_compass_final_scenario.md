...
### 9.6 Document Intelligence (Agent Bricks 외 layer, production cron pattern)
- **`ai_parse_document()` SQL function** — UC Volume PDF → markdown text (Databricks Document Intelligence parser GA, 2026-04)
- **Foundation Model API 직접 호출** (`databricks-claude-haiku-4-5`) — parsed text → structured fields (saudi_production_kbbl_d / iran / opec_total / forecast_demand) 추출
- **적재**: `bronze.opec_momr_parsed` (74 rows, 2020-01 ~ 2026-03)
- **Pipeline**: `job_opec_momr_monthly` (cron `0 0 12 * *`) — 매월 OPEC MOMR 발표 직후 incremental (1 row/month, 새 PDF만 처리)
- **데모 anchor**: "OPEC 4월호 발표 → cron 실행 → 사우디 감산 % 자동 추출 → `gold.opec_demand_gap` view 즉시 reflection → Pattern Score 갱신"

**Agent Bricks Information Extraction과 차이** (5/17 D-1 결정):
- IE agent UI에서 schema-defined extraction 검증 완료 (`crude-compass-ie` agent 5 sample PDFs 처리 확인)
- 그러나 우리 use case (single-source OPEC MOMR + single-schema)에서는 **redundant** — bronze 적재 시 이미 동일 fields 추출 + monthly incremental로 비용 효율
- IE를 silver layer 추가 시 매월 74 rows full refresh 부담 → DLT pipeline 필요 → D-2 critical path 위해 scope-out
- **D-1 buffer 시 검토**: DLT pipeline + IE agent로 swap 시 schema-managed governance + Auto Loader incremental 가능
...
- **2 Agent Bricks types 활용** (Knowledge Assistant + Supervisor) — Hackathon 4-tool Agent Bricks 카테고리 충족. Information Extraction은 우리 single-source single-schema use case에 redundant 판단으로 scope-out (§9.6 참조).
- **Supervisor Agent endpoint** (`crude-compass-supervisor`): OpenAI chat completions 호환, `databricks_options.return_trace=true` 활성화
- **3 Sub-agent 자동 라우팅**:
  1. **Genie Space** (`crude-compass-genie`) — 정형 데이터 SQL 자연어 (silver/gold tables 포함 OPEC supply 정량 fields)
  2. **Knowledge Assistant** (`ka-crude-compass-ka`) — OPEC MOMR PDF RAG + citation (page number 포함)
  3. **Mission Plan FMA** (`databricks-claude-haiku-4-5`) — Bidirectional Mission 권고 (HEDGE/OPP/Pivot reasoning)
- **단일 endpoint orchestration**: 평가위원 자연어 1개 → 3 sub-agent 적절히 delegate → 종합 답변 + tools_used trace
...
- **IE scope-out 솔직 narrative** (5/17 D-1): Information Extraction agent UI 검증 후 우리 use case 측면 redundant 판단 — `bronze.opec_momr_parsed`가 이미 `ai_parse_document` + Foundation Model API 직접 호출로 정량 fields (saudi/iran/opec_total/forecast_demand) 추출 완료. IE schema-defined extraction 추가는 매월 74 rows full refresh 부담 + 동일 fields 중복 의미. D-1 buffer 시 DLT pipeline incremental으로 upgrade 검토.
...
| 2 | 4 | 자연어 1개 → **Agent Bricks Supervisor**가 3 sub-agent (Genie + KA + FMA Mission Plan) 자동 라우팅 → 종합 답변 + tools_used trace 표시 (single endpoint governed orchestration) | 우리 차별화 ⭐⭐ |
...
| 4 | Technical Capability | **15-17/20** | 공식 4 features 모두 production-grade: Apps (deploy + Git source 자동 build) + Lakebase (OAuth pool) + Genie (4-tier fallback) + AgentBricks 2 types (Knowledge Assistant + Multi-Agent Supervisor with 3 sub-agent + return_trace). 추가: Foundation Model API 직접 호출, UC Function, Document Intelligence (`ai_parse_document` + FMA chain), Lakeflow 8 jobs UNPAUSED |
