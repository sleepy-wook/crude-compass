...
3. 단계별 추론이 스트리밍됨 → 최종 답변 → 하단에 "AI 분석 단계: Genie · Knowledge Assistant" 표시.
   ※ 실제 trace는 "어떤 에이전트를 거쳤는지"까지만 노출(에이전트별 상세 행동 X). 멘트도 거기 맞춤.

**내레이션:** (실제 화면이 보여주는 만큼만 — 과장 X)

> 마지막으로, 더 깊이 파고들고 싶으면 조사 탭에서 자연어로 직접 묻습니다. 이번 주 러시아 신호와 OPEC 수급 전망을 묶어, Term을 어떻게 가야 할지 물어보겠습니다.
>
> Agent Bricks Supervisor가 단계적으로 추론하면서, Genie로 시장과 신호 데이터를 조회하고 지식 도우미로 OPEC 보고서를 찾아 답을 종합합니다. 답변 끝에는 어떤 sub-agent를 거쳤는지가 표시됩니다.

> [체크] Supervisor·Genie·KA 엔드포인트는 실측 READY 확인됨(`mas-ba3fbcb5` / `01f1…034` / `ka-6b456458`). 녹화 전 조사 탭에서 위 질문으로 trace에 Genie+KA 둘 다 뜨는지만 한 번 확인. 미라우팅 시 Genie fallback으로라도 동작.
