...
- Databricks Apps · Genie · Lakebase Autoscaling (GA) · Agent Bricks Custom Agents (2026-02-18 GA)
- Frontend: **Vite + React 18 + TypeScript + Tailwind** (shadcn/ui · Framer Motion 선별 도입)
- Backend: **FastAPI 단일 프로세스** (Vite dist 정적 serve + `/api` 라우트, 단일 컨테이너 단일 포트)
...
| Source | 데이터 | 검증 |
|---|---|---|
| **OilPriceAPI** (Developer $19/월) | WTI + Brent + **Dubai** + 가솔린·디젤 (REST 5분 갱신, Tier 2 cron) | ✅ 2026-05-06 |
| **AIS aisstream** | 호르무즈 통과량 + 익명 charter VLCC 선단 (WebSocket → **Lakeflow Workflow continuous task**) — 공공 vessel data | ✅ 2026-05-03 |
| **GDACS** | 글로벌 재해 events (REST, no auth) — UN/EU 공식 | ✅ 2026-05-06 |
| **ECOS 한국은행** | KRW/USD 환율 종가 (REST) — 한국은행 공공 | ✅ 2026-05-06 |
...
- AIS WebSocket = **Lakeflow Workflow continuous task** (Apps 외부, 24/7 streaming insert into bronze.ais_positions)
  - 호르무즈 BoundingBox subscribe + 익명 charter VLCC MMSI subscribe
  - Apps 안에 두지 않음 (Apps scale-to-zero 정책 + 공식 가이드 없음, 2026-05 검증)
...
| Tool | 역할 | Wow 모먼트 |
|---|---|---|
| **Apps** | Vite + React + FastAPI 단일 프로세스<br>3 page (Discovery / Mission / Yesterday Review) | desktop-first 1280px primary |
...
| 6 | 03:30~04:00 | **Mission 인라인 Genie 시뮬** ⭐ | "이 미션에 Brent $140·Dubai $135"이면?" → ROI 차트 (Mission 페이지 안) |
| 7 | 04:00~04:30 | Yesterday Review + Genie + **AI/BI Dashboard** | 어제 결정 outcome + Self-critique + Genie "다시 묻기" + 30일 시계열 dashboard |
...
**🎬 Wow 3 (Phase 6) — Mission 인라인 Genie 시뮬** (Genie 빛남)
- Mission 컨텍스트 안에서 한국어 NL → Lakebase + Gold tables 종합 reasoning
- polling 1~5s + 24h cache + pre-warmed 시나리오 fallback chain (5초 SLA 부재 대응)
- "What-If" 별도 페이지 → Mission 인라인으로 이전 (실제 워크플로우와 자연스럽게 결합)
...
- ✅ Agent Bricks Custom Agents 2026-02-18 GA (Beta 아님, 공식 docs 검증)
- ✅ Lakebase Autoscaling GA (2026-03)
- ✅ **Vite + React + FastAPI 단일 프로세스** = Apps deploy 단순 (Day-1 PoC 검증 예정)
- ⚠️ Lakebase OAuth 토큰 refresh wrapper 작성 (PGPASSWORD 미주입, SDK 토큰 발급)
- ⚠️ Genie Conversation API 5초 SLA 부재 → 4중 fallback chain (polling + 24h cache + pre-warmed + 정적)
- ⚠️ `databricks_langchain.genie` multi-agent **Public Preview** (Plan A) / Genie와 Custom Agents 별도 surface (Plan B)
- ⚠️ AIS WebSocket = Lakeflow Workflow continuous task (Apps 외부, 검증 완료)
