...
# MAGIC %md
# MAGIC # Backtest LLM v6 — Signal Recency + Structured Fields (C + D)
# MAGIC
# MAGIC ## v5 → v6 변경점 (5/13)
# MAGIC
# MAGIC ### C — Signal Recency Weighting (prompt 구조)
# MAGIC v5: 90일 top 15 신호를 importance순으로 단일 리스트로 LLM에 전달
# MAGIC v6: **시간 버킷별 명시** ("최근 7일", "8-30일", "31-90일") + 각 버킷의 핵심 신호
# MAGIC →  LLM이 regime shift를 catch하기 쉬워짐 (예: 2020-11 vaccine rally)
# MAGIC
# MAGIC ### D — Structured Fields (정량 데이터 명시)
# MAGIC v5: signal list만 (LLM이 추론으로 정량값 도출)
# MAGIC v6: prompt에 명시:
...
# MAGIC %md
# MAGIC ## Step 1: Signals view (v5와 동일)

# COMMAND ----------

spark.sql("""
CREATE OR REPLACE TEMP VIEW _v6_signals AS
WITH gdelt AS (
...
spark.sql("""
    CREATE OR REPLACE TEMP VIEW _v6_dubai AS
...
# Pattern Score (v5 동일)
spark.sql(f"""
CREATE OR REPLACE TEMP VIEW _v6_pattern_daily AS
WITH date_dim AS (
...
contribs AS (
    ...
    FROM date_dim d JOIN _v6_signals s
...
# MAGIC %md
# MAGIC ## Step 2: Stratified sampling (v5와 동일)
...
all_rows = spark.sql("""
    ...
    FROM _v6_pattern_daily p
    JOIN _v6_dubai b ON p.as_of_date = b.price_date
    WHERE p.as_of_date <= (SELECT DATE_SUB(MAX(price_date), 90) FROM _v6_dubai)
""").collect()
...
def fetch_context_v6(as_of_date):
    """v6: 시간 버킷 + structured fields 포함."""
    # Pattern Score
    ps_rows = spark.sql(f"""
        SELECT pattern_score, bullish, bearish, sig_count, cross_val_bonus
        FROM _v6_pattern_daily WHERE as_of_date = DATE'{as_of_date}'
    """).collect()
    ...
    dubai = spark.sql(f"""
        WITH d AS (
            SELECT price_date, dubai_close FROM _v6_dubai
    ...
    out_rows = spark.sql(f"""
        SELECT
            (SELECT dubai_close FROM _v6_dubai WHERE price_date = DATE'{as_of_date}') AS d0,
            (SELECT dubai_close FROM _v6_dubai WHERE price_date >= DATE'{as_of_date}' + INTERVAL 7 DAYS  ORDER BY price_date LIMIT 1) AS d7,
            (SELECT dubai_close FROM _v6_dubai WHERE price_date >= DATE'{as_of_date}' + INTERVAL 30 DAYS ORDER BY price_date LIMIT 1) AS d30,
            (SELECT dubai_close FROM _v6_dubai WHERE price_date >= DATE'{as_of_date}' + INTERVAL 90 DAYS ORDER BY price_date LIMIT 1) AS d90
    """).collect()[0]
