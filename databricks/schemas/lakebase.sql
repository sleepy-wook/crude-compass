...
-- ====================================================================
-- 적재: FastAPI (reports / daily_reports) + backtest_predictions (1회 seed, static analog).
...
-- ────────────────────────────────────────────────────────────────────
-- backtest_predictions — 7년 historical analog (market memory)
-- ────────────────────────────────────────────────────────────────────
-- Read: market memory (find_similar_patterns) — Investigation/의사결정 "비슷한 시그널" 참고.
-- Note: 1회 seed된 static analog. 과거 데이터라 갱신 불필요 (writer job 제거됨).
...
-- ────────────────────────────────────────────────────────────────────
-- reports — Event-driven AI report inbox (2026-05-21 reports model)
-- ────────────────────────────────────────────────────────────────────
-- daily AI workflow의 핵심. 매 trigger (gdelt_signal / price_spike / pattern_drift)마다 1 row.
-- status flow: pending → kept | dropped | ai_dropped | archived
...
-- ────────────────────────────────────────────────────────────────────
-- daily_reports — 매일 06:30 KST cron 종합 보고서 + 비중 제안
-- ────────────────────────────────────────────────────────────────────
